...
try:
    print('\nloading weights from {}\n'.format(C.base_net_weights))
    model_rpn.load_weights(C.base_net_weights, by_name=True)
    model_classifier.load_weights(C.base_net_weights, by_name=True)
except :
    raise Exception('Could not load pretrained model weights. Weights can be found in the keras application folder \
    ...
    while True:
        try:
            ...
            if len(rpn_accuracy_rpn_monitor) == epoch_length and C.verbose:
                ...
                rpn_accuracy_rpn_monitor = []
                print('\nAverage number of overlapping bounding boxes from RPN = {} for '
                      '{} previous iterations'.format(mean_overlapping_bboxes, epoch_length))
            ...
            # print('\nneg:', len(neg_samples[0]))
            # print('pos:', len(pos_samples[0]))
